...
contract Runescape is ERC1155, Ownable, ERC1155Supply {
  ...
  function registerPlayer(string memory _name, string memory _gameTokenName) external {
    ...
    players.push(Player(msg.sender, _name, 10, false)); // Adds player to players array
  ...
  function _registerPlayerMove(uint256 _player, uint8 _choice, string memory _battleName) internal {
    require(_choice == 1 || _choice == 2 || _choice == 3, "Choice should be either 1 or 2 or 3!");

    battles[battleInfo[_battleName]].moves[_player] = _choice;
  }

  // User chooses attack or defense move for battle card
  function MeleeMageOrRange(uint8 _choice, string memory _battleName) external {
    Battle memory _battle = getBattle(_battleName);
  ...
  function _resolveBattle(Battle memory _battle) internal {
    ...
    // in the combat triangle, melee beats range, range beats mage, mage beats melee
    ...
    if (p1.move == 1 && p2.move == 1) {
      if (1 == p2.health) {
        ...
        players[p1.index].playerHealth -= 1;
        players[p2.index].playerHealth -= 1;
